...
public class WebServicesTablePagePropertyViewPart extends AbstractSinglePageSectionBasedViewPart {
  ...
  protected void createSections() {
    ...
    try {
      ...
      action.init(getPage().getMarkerGroupUUID(), getPage().getScoutResource());
      AbstractPresenter presenter = new ActionPresenter(getSection(SECTION_ID_REPAIR).getSectionClient(), action, getFormToolkit());
      ...
      if (JaxWsSdkUtility.exists(buildJaxWsFile)) {
        ...
        presenter = new ActionPresenter(getSection(SECTION_ID_LINKS).getSectionClient(), a, getFormToolkit());
        applyLayoutData(presenter);
      }
      ...
      b.init(m_bundle);
      presenter = new ActionPresenter(getSection(SECTION_ID_LINKS).getSectionClient(), b, getFormToolkit());
      applyLayoutData(presenter);
  ...
  private class P_ScoutSeverityListener implements IScoutSeverityListener {
    ...
    public void severityChanged(IResource resource) {
      if (resource == ResourceFactory.getBuildJaxWsResource(m_bundle).getFile()) {
