...
public final class ResourcesUtility {
	 ...
	 * <p>
	 * This default line delimiter should be used by clients who want unique
	 * delimiters (e.g. 'CR's) in the document.
	 * </p>
	 * 
	 ...
	 * 
	 * @param baseDir
	 *            The base dir. All files (recursively) in this folder will be
	 *            added to the zip.
	 * @param zOut
	 *            The zip where the files will be added.
	 * @throws IOException
	 */
	public static void addFolderToZip(File baseDir, ZipOutputStream zOut)
			throws IOException {
		addFolderToZipRec(baseDir, baseDir, zOut);
	}

	private static void addFolderToZipRec(File baseDir, File file,
			ZipOutputStream zOut) throws IOException {
		if ((!file.exists()) || (!file.isDirectory())) {
			throw new IOException("source directory " + file
	...
	/**
	 * returns the line separator defined in preference
	 * {@link org.eclipse.core.runtime.Platform#PREF_LINE_SEPARATOR} on the
	 * workspace. If this is null, returns the system line separator.
	 * 
	 ...
	 * returns the line separator defined in preference
	 * {@link org.eclipse.core.runtime.Platform#PREF_LINE_SEPARATOR} on the
	 * project or workspace of the given resource. If this is null, returns the
	 * platform separator.
	 * 
	 * @return The line separator to use.
	 */
	public static String getLineSeparator(IResource r) {
		if (r != null) {
			IScopeContext[] scopeContext = new IScopeContext[] { new ProjectScope(
					r.getProject()) };
	...
	/**
	 * Finds and returns the recommended line separator for this element. The
	 * element's buffer is first searched and the first line separator in this
	 * buffer is returned if any. Otherwise the preference
	 * {@link org.eclipse.core.runtime.Platform#PREF_LINE_SEPARATOR} on this
	 * element's project or workspace is returned. Finally if no such preference
	 * is set, the system line separator is returned.
	 * 
	 ...
	 */
	public static void mkdirs(IFile toCreate, IProgressMonitor monitor)
			throws CoreException {
	 ...
	 * @throws CoreException
	 */
	public static void mkdirs(IContainer toCreate, IProgressMonitor monitor)
			throws CoreException {
		if (toCreate == null || toCreate.exists()) {
			return;
		} else {
