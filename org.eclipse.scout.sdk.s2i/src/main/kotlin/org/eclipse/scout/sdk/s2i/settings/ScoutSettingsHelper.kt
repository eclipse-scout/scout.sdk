...
object ScoutSettingsHelper {
    ...
    fun isAutoUpdateDerivedResources(project: Project) =
        projectSettings(project).getBoolean(ScoutSettings.KEY_AUTO_UPDATE_DERIVED_RESOURCES, true)

    fun setAutoUpdateDerivedResources(project: Project, newValue: Boolean) =
        changeProperty(projectSettings(project), ScoutSettings.KEY_AUTO_UPDATE_DERIVED_RESOURCES, isAutoUpdateDerivedResources(project).toString(), newValue.toString())

    fun isAutoCreateClassIdAnnotations(project: Project) =
        projectSettings(project).getBoolean(ScoutSettings.KEY_AUTO_CREATE_CLASS_ID, false)
    ...
    fun setTranslationLanguage(project: Project, language: Language?) {
        ...
        changeProperty(projectSettings(project), ScoutSettings.KEY_TRANSLATION_DEFAULT_LANG, oldName, name)
    }

    private fun projectSettings(project: Project): PropertiesComponent = PropertiesComponent.getInstance(project)
