...
public class AnnotationSourceTest {
  ...
  public void testAnnotationWithDefaultValuesInSourceFile() {
    ...
    Assert.assertEquals(1, t.getAnnotations().size());
    IAnnotation a = t.getAnnotations().get(0);
  ...
  public void testAnnotationWithSingleValuesInSourceFile() {
    ...
    IMethod m = t.getMethods().get(0);//the constructor is a synthetic method
    Assert.assertEquals(1, m.getAnnotations().size());
    a = m.getAnnotations().get(0);
  ...
  public void testAnnotationWithArrayValuesInSourceFile() {
    ...
    //deep check
    IAnnotation annos0 = a.getValue("annos").getMetaValue().getObject(IAnnotation[].class)[0];
    ...
    IAnnotation annos1 = a.getValue("annos").getMetaValue().getObject(IAnnotation[].class)[1];
    ...
    //deep check
    annos0 = a.getValue("annos").getMetaValue().getObject(IAnnotation[].class)[0];
    ...
    annos1 = a.getValue("annos").getMetaValue().getObject(IAnnotation[].class)[1];
  ...
  public void testAnnotationWithSingleValuesInBinaryFile() {
    ...
    IMethod m = CoreUtils.getMethod(t, "run");
    Assert.assertEquals(1, m.getAnnotations().size());
    a = m.getAnnotations().get(0);
  ...
  private static void assertAnnotationValue(IAnnotation a, String key, MetaValueType valueType, Class<?> modelType, String aptSource, String expressionSource) {
    IAnnotationValue av = a.getValue(key);
    IMetaValue mv = av.getMetaValue();
    Assert.assertEquals(valueType, mv.getType());
    Assert.assertEquals(modelType, mv.getObject(Object.class).getClass());
    ...
    ISourceRange range = av.getSourceOfExpression();
  ...
  private static void assertAnnotationArrayValues(IAnnotation a, String key, MetaValueType elementValueType, Class<?> elementModelType, String[] aptSources, String expressionSource) {
    IAnnotationValue av = a.getValue(key);
    IMetaValue mv = av.getMetaValue();
    Assert.assertEquals(MetaValueType.Array, mv.getType());
    Assert.assertTrue(mv instanceof IArrayMetaValue);
    IMetaValue[] elements = ((IArrayMetaValue) mv).getMetaValueArray();
    Assert.assertEquals(aptSources.length, elements.length);
    for (int i = 0; i < elements.length; i++) {
      Assert.assertEquals(elementValueType, elements[i].getType());
      Assert.assertEquals(elementModelType, elements[i].getObject(Object.class).getClass());
