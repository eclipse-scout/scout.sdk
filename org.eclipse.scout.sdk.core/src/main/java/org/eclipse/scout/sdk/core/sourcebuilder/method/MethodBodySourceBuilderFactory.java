  ...
  public static IMethodBodySourceBuilder createAutoGeneratedMethodBody() {
    return new IMethodBodySourceBuilder() {
      ...
      public void createSource(IMethodSourceBuilder methodBuilder, StringBuilder source, String lineDelimiter, PropertyMap context, IImportValidator validator) {
        ...
        if (!StringUtils.isEmpty(returnTypeSignature) && !Signature.SIG_VOID.equals(returnTypeSignature)) {
  ...
  public static IMethodBodySourceBuilder createSuperCallMethodBody(final boolean addAutoGeneratedMethodStubComment) {
    return new IMethodBodySourceBuilder() {
      ...
      public void createSource(IMethodSourceBuilder methodBuilder, StringBuilder source, String lineDelimiter, PropertyMap context, IImportValidator validator) {
        ...
        source.append("(");
        List<MethodParameterDescription> parameters = methodBuilder.getParameters();
        if (parameters.size() > 0) {
          MethodParameterDescription param = parameters.get(0);
          source.append(param.getName());
