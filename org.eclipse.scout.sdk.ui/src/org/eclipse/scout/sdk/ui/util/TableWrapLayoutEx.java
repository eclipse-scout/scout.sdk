 ...
 * <p>
 * The advantage of the algorithm over GridLayout is that it is capable of flowing text controls capable of line wrap.
 * These controls do not have natural 'preferred size'. Instead, they are capable of providing the required height if
 * the width is set. Consequently, this algorithm first calculates the widths that will be assigned to columns, and then
 * passes those widths to the controls to calculate the height. When a composite with this layout is a child of the
 * scrolling composite, they should interact in such a way that reduction in the scrolling composite width results in
 * the reflow and increase of the overall height.
 * <p>
 * If none of the columns contain expandable and wrappable controls, the end-result will be similar to the one provided
 * by GridLayout. The difference will show up for layouts that contain controls whose minimum and maximum widths are not
 * the same.
 * 
 * @see TableWrapDataEx
 * @since 3.0
...
public final class TableWrapLayoutEx extends Layout implements ILayoutExtension {
  ...
  private class RowSpan {
    ...
    public void update(int currentRow, int rowHeight) {
      TableWrapDataEx td = (TableWrapDataEx) child.getLayoutData();
      // is currentRow within this span?
      if (currentRow >= row && currentRow < row + td.rowspan) {
        totalHeight += rowHeight;
        if (currentRow > row) totalHeight += verticalSpacing;
      }
    }

    public int getRequiredHeightIncrease() {
      if (totalHeight < height) return height - totalHeight;
      return 0;
    }
  }
  ...
  public int computeMinimumWidth(Composite parent, boolean changed) {
    ...
    if (minColumnWidths == null) minColumnWidths = new int[numColumns];
  ...
  public int computeMaximumWidth(Composite parent, boolean changed) {
    ...
    if (maxColumnWidths == null) maxColumnWidths = new int[numColumns];
    for (int i = 0; i < numColumns; i++) {
      maxColumnWidths[i] = 0;
    }
  ...
  protected void layout(Composite parent, boolean changed) {
    ...
    if (parentWidth <= minWidth) {
      ...
      else columnWidths = minColumnWidths;
    }
  ...
  int[] computeRowHeights(Control[] children, int[] columnWidths,
    ...
    for (Enumeration enm = rowspans.elements(); enm.hasMoreElements();) {
      ...
      TableWrapDataEx td = (TableWrapDataEx) rowspan.child.getLayoutData();
      int ngrowing = 0;
  ...
  Point computeSize(int childIndex, int width, int indent, int maxWidth, int maxHeight) {
    ...
    if (!isWrap(controlCache.getControl())) widthArg = SWT.DEFAULT;
    Point size = controlCache.computeSize(widthArg, SWT.DEFAULT);
    if (maxWidth != SWT.DEFAULT) size.x = Math.min(size.x, maxWidth);
    if (maxHeight != SWT.DEFAULT) size.y = Math.min(size.y, maxHeight);
    size.x += indent;
    return size;
  }

  void placeControl(Control control, TableWrapDataEx td, int x, int y,
    ...
    if (rowspan != null) {
      ...
      for (int i = row; i < row + td.rowspan; i++) {
        if (i > row) slotHeight += verticalSpacing;
        slotHeight += rowHeights[i];
      }
    }
    // align horizontally
    if (td.align == TableWrapDataEx.CENTER) {
      xloc = x + colWidth / 2 - width / 2;
    }
    else if (td.align == TableWrapDataEx.RIGHT) {
      xloc = x + colWidth - width;
    }
    else if (td.align == TableWrapDataEx.FILL) {
      width = colWidth;
    }
    // align vertically
    if (td.valign == TableWrapDataEx.MIDDLE) {
      yloc = y + slotHeight / 2 - height / 2;
    }
    else if (td.valign == TableWrapDataEx.BOTTOM) {
      yloc = y + slotHeight - height;
    }
    else if (td.valign == TableWrapDataEx.FILL) {
      height = slotHeight;
    }
  ...
  void createGrid(Composite composite) {
    ...
    for (int i = 0; i < children.length; i++) {
      ...
      while (((TableWrapDataEx[]) grid.elementAt(row))[column] != null) {
        column = column + 1;
      ...
      ((TableWrapDataEx[]) grid.elementAt(row))[column] = spec;
      spec.childIndex = i;
      ...
      for (int r = 1; r <= rowFill; r++) {
        for (int c = 0; c < spec.colspan; c++) {
          ...
          ((TableWrapDataEx[]) grid.elementAt(row + r))[column + c] = spacerSpec;
        }
      }
    ...
    for (int k = column + 1; k < numColumns; k++) {
      spacerSpec = new TableWrapDataEx();
      spacerSpec.isItemData = false;
      ((TableWrapDataEx[]) grid.elementAt(row))[k] = spacerSpec;
    }
    for (int k = row + 1; k < grid.size(); k++) {
      spacerSpec = new TableWrapDataEx();
      spacerSpec.isItemData = false;
      ((TableWrapDataEx[]) grid.elementAt(k))[column] = spacerSpec;
    }
  ...
  private TableWrapDataEx[] createEmptyRow() {
    TableWrapDataEx[] row = new TableWrapDataEx[numColumns];
    for (int i = 0; i < numColumns; i++)
      row[i] = null;
    return row;
  }
  ...
  protected Point computeSize(Composite parent, int wHint, int hHint,
    ...
    if (wHint == SWT.DEFAULT) parentWidth = maxWidth;
    ...
    if (!rowspans.isEmpty()) innerHeight = compensateForRowSpans(innerHeight);
  ...
  int internalGetMinimumWidth(Composite parent, boolean changed) {
    ...
    for (int i = 0; i < numColumns; i++) {
      ...
      if (makeColumnsEqualWidth) minimumWidth += widestColumnWidth;
      else minimumWidth += minColumnWidths[i];
    }
    // add margins
  ...
  void calculateColumnWidths(Composite parent, int[] columnWidths, boolean max, boolean changed) {
    ...
    for (int i = 0; i < grid.size(); i++) {
      ...
      for (int j = 0; j < numColumns; j++) {
        ...
        if (td.maxWidth != SWT.DEFAULT) width = Math.min(width, td.maxWidth);
    ...
    for (int i = 0; i < grid.size(); i++) {
      ...
      for (int j = 0; j < numColumns; j++) {
        TableWrapDataEx td = row[j];
        if (td.isItemData == false || td.colspan == 1) continue;
        ...
        if (width <= current) {
          ...
          if (ndiv == 0) {
            ...
            if ((width - current) % ndiv > 0) percolumn++;
            for (int k = j; k < j + td.colspan; k++) {
              if (isGrowingColumn(k)) columnWidths[k] += percolumn;
            }
          }
        }
      }
  ...
  void initializeLayoutData(Composite composite) {
    Control[] children = getOrderdVisibleChildren(composite);
    for (int i = 0; i < children.length; i++) {
      Control child = children[i];
      if (child.getLayoutData() == null) {
        child.setLayoutData(new TableWrapDataEx());
      }
    }
  }
  ...
  protected Control[] getOrderdVisibleChildren(Composite parent) {
    ...
    for (Iterator<Control> it = children.iterator(); it.hasNext();) {
      ...
      if (layoutData instanceof TableWrapDataEx) {
        if (((TableWrapDataEx) layoutData).exclude) {
          it.remove();
        }
      }
    }
