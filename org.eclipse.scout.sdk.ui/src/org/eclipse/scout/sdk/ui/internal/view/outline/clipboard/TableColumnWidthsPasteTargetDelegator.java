...
public class TableColumnWidthsPasteTargetDelegator implements IPasteTargetDelegator {
  ...
  private void showInfoMessageBox(AbstractPage page, String message) {
    // TODO [sme] information icon
    MessageBox msgBox = new MessageBox(page.getOutlineView().getSite().getShell(), SWT.OK | SWT.ICON_INFORMATION);
  ...
  private String getStringFromTransferable(Transferable contents) {
    ...
    Reader reader;
    try {
      reader = DataFlavor.stringFlavor.getReaderForText(contents);
    ...
    try {
      ...
      for (int i = 1; i < lines.length; i++) { // start with second row, because first row contains identifier
        ...
        if (StringUtility.hasText(line)) {
          ...
          if (columnWidth < 0) {
            showInfoMessageBox(page, Texts.get("ColumnWidthPasteInvalidWidth", columnWidth.toString()));
            return null;
          }
      ...
      // no error log, because content is not parsable
  ...
  private IType determineTableType(AbstractPage page, Set<String> classNames) {
    ...
    if (page instanceof ColumnTablePage) {
      ...
      if (tables.length > 0) {
        ...
        showInfoMessageBox(page, Texts.get("ColumnWidthPasteNoTableInPage"));
      }
    }
    else {
      ...
      if (page instanceof TableFieldNodePage) {
        ...
        if (tables.length > 0) {
          ...
          showInfoMessageBox(page, Texts.get("ColumnWidthPasteNoTableInField"));
        }
      }
        ...
        showInfoMessageBox(page, Texts.get("ColumnWidthPasteUnsupportedSelection"));
      }
    }
    ...
    if (tableType != null) {
      ...
      for (String clazz : classNames) {
        if (!clazz.startsWith(tableTypeClassName)) {
          ...
          showInfoMessageBox(page, Texts.get("ColumnWidthPasteDifferentTable", tableTypeClassName, clazz));
          tableType = null;
          break;
        }
      }
