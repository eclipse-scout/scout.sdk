...
public class LookupServiceProposalPresenter extends AbstractTypeProposalPresenter {
  ...
  protected IType[] provideScoutTypes(IJavaProject project, IType ownerType) {
    return TypeUtility.getPrimaryTypeHierarchy(iLookupService).getAllSubtypes(iLookupService, TypeFilters.getTypesOnClasspath(project), TypeComparators.getTypeNameComparator());
  }
...
//        final IType lookupServiceInterface = TypeUtility.getType(lookupCall.getPackageFragment().getElementName() + ".I" + entityName + "Service");
...
//          });
//        }
//        final IType lookupServiceImplementation = TypeUtility.getType(lookupCall.getPackageFragment().getElementName().replace(".shared.", ".server.") + "." + entityName + "Service");
