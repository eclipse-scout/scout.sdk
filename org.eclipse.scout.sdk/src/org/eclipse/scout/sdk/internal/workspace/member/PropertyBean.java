 ...
 ******************************************************************************/
package org.eclipse.scout.sdk.internal.workspace.member;
  ...
  public String getBeanSignature() {
    ...
    if (TypeUtility.exists(getReadMethod())) {
      try {
        ...
        beanSignature = ScoutSdkUtility.getQuallifiedSignature(beanSignature, getReadMethod().getDeclaringType());
        return beanSignature;
      }
      catch (JavaModelException e) {
        ScoutSdk.logWarning("could not parse signature of '" + getReadMethod().getElementName() + "' in type '" + getReadMethod().getDeclaringType().getFullyQualifiedName() + "'.", e);
      }
    }
    if (TypeUtility.exists(getWriteMethod())) {
      try {
        ...
        if (paramSignature != null && paramSignature.length == 1) {
          ...
          beanSignature = ScoutSdkUtility.getQuallifiedSignature(beanSignature, getWriteMethod().getDeclaringType());
          return beanSignature;
        }
      }
      catch (JavaModelException e) {
        ScoutSdk.logWarning("could not parse signature of '" + getWriteMethod().getElementName() + "' in type '" + getWriteMethod().getDeclaringType().getFullyQualifiedName() + "'.", e);
      }
    }
    return null;
  }
