...
/**
 * Create a server development product file and config.ini (for localhost name) based on a production product /
 * config.ini
 */
public class CreateServerDevelopmentProductOperation implements IOperation {
   ...
   */
  public CreateServerDevelopmentProductOperation(IFile templateProductFile, IFile devProduct) {
  ...
  public void validate() throws IllegalArgumentException {
    if (m_templateProductFile == null) {
      throw new IllegalArgumentException("template product file can not be null.");
    }
    if (!m_templateProductFile.exists()) {
      throw new IllegalArgumentException("template product file does not exist.");
    }
  }
      ...
      if (devConfigIni.exists()) {
        ...
        for (String s : configIniContent) {
          ...
          if (!s.startsWith("#") && a.length == 2) {
            String key = a[0].trim();
            // change remote file path
            if (key.equals("org.eclipse.scout.rt.server.services.common.file.RemoteFileService#rootPath")) {
              String fsRoot = new File(devConfigIni.getProject().getLocation().toOSString()).getAbsolutePath().replace('\\', '/');
              newContent.add(key + "=" + fsRoot + "/j2ee/ear/war/WEB-INF/remotefiles");
            }
            // default add
            else {
              newContent.add(s);
          ...
          else if ("#Eclipse Runtime Configuration File".equals(s)) {
            // ignore
          }
          else {
            newContent.add(s);
