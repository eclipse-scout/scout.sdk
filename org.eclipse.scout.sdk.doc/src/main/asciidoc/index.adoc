...
The `org.eclipse.scout.sdk.core.model.api.IAnnotation` represents a Java annotation which may be present on any `IAnnotatable` element (<<Members>>, `IMethodParameter`, `IPackage`).

It may contain `IAnnotationElement` instances in case the annotation has attributes. `IAnnotationElement` instances have a value of type `IMetaValue` which corresponds to a compile-time constant (see <<Constants>>).
...
. The `Api` class know knows that the `IJavaApi` is available and uses version `8`.
...
. The `ApiSpecification` reads the `ApiLevel` annotation of the two specification classes and discards all which have a level higher than the version requested.
...
* Annotations (see <<Annotations>>) like `ClassIdAnnotation`, `FormDataAnnotation` or `OrderAnnotation` to parse Scout annotation more conveniently.
* Source builders (see <<Source Builders>>) like `ScoutMethodBodyBuilder` which add Scout specific source fragments like `BEANS.get(Class)` calls.
* Generators (see <<Generators>>) for various Scout elements like `CodeTypeGenerator`, `FormGenerator`, `ScoutAnnotationGenerator` or `LookupCallGenerator`.
...
* Environment: A strategy that must be implemented to execute operations (see <<Operations>>).
Only an interface is provided.
* `ClassIds`: A registry of class name dependent @ClassId providers.
