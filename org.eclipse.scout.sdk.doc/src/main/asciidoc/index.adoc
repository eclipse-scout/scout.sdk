...
If you like to help, please create a Pull Request for repository https://github.com/eclipse-scout/scout.sdk.
Thanks!
...
It contains classes to deal with source code in general.
Furthermore, various utility classes are provided to support common use cases (like XML parsing or string operations).
This module contains no Scout specific or programming language specific code.

There are additional core modules for Java and TypeScript source parsing and source creation.
While the Java module contains all necessary code to parse Java source code (provided by `ecj`), the TypeScript module only contains a service provider interface definition (SPI) and requires an external parser which implements this interface.

The `org.eclipse.scout.sdk.core.s` (please note the extra `s` at the end) introduces Scout specific classes, e.g. to create a FormData or Page (orange in <<img-overview>>).

For all parts corresponding `test` modules are available holding classes to support unit testing.
...
The `core.java` modules provide classes for two main topics:

. Parse Java code into a model representation.
. Build new Java source code.

These two topics are explained in the next chapters.

==== Parsing

The package `org.eclipse.scout.sdk.core.java.model` contains classes that represent Java code (source or binary bytecode) as a model.
...
The main entry point is the `org.eclipse.scout.sdk.core.java.model.api.IJavaEnvironment` which represents a Java class path.
...
An `org.eclipse.scout.sdk.core.java.model.api.IType` represents a Java type (e.g. class, interface, enum, array or primitive type) and provides access to super classes or interfaces, inner (nested) types, annotations, type-parameters, methods, fields and more.
...
The `org.eclipse.scout.sdk.core.java.model.api.ICompilationUnit` represent a `*.java` file and provides access to the imports (`IImport`) and `IPackage` and may contain multiple `IType` instances.
...
The `org.eclipse.scout.sdk.core.java.model.api.IAnnotation` represents a Java annotation which may be present on any `IAnnotatable` element (<<Member>>, `IMethodParameter`, `IPackage`).
...
----
include::{codedir}/PrintAnnotationElementValue.java[tags=printAnnotationElementValue]
----

<1> Gets the annotations for which the annotation type matches the `ClassIdAnnotation` model (in this case `org.eclipse.scout.rt.platform.classid.ClassId`) and wraps the `IAnnotation` into an instance of `ClassIdAnnotation` (typed).
<2> The `ClassIdAnnotation` has named getters like `value()` in this example which correspond to the matching annotation attribute.
...
The `org.eclipse.scout.sdk.core.java` module contains the Java model implementation including queries, annotations and various other helper classes as explained before.
...
The SPI can be found in the package `org.eclipse.scout.sdk.core.java.model.spi`.
The model functionality is then built on top of the SPI instances.

Currently, there is one SPI implementation available.
It exists in the `org.eclipse.scout.sdk.core.java.ecj` module and uses the https://github.com/eclipse-jdt/eclipse.jdt.core/wiki/ECJ[Eclipse Compiler for Java] as parser.
...
. Transformers: A transformer is like an adapter that converts a parsed Java model (see <<core-java>>) into a modifiable generator.
...
There is e.g. a source builder for Java code (`org.eclipse.scout.sdk.core.java.builder.IJavaSourceBuilder`) which adds methods for Java specific source elements like a semicolon or a reference to a Java type.
...
The Java model classes (see <<core-java>>) can be converted to `ISourceGenerator` instances to apply modifications.
...
Import collectors (base interface `org.eclipse.scout.sdk.core.java.imports.IImportCollector`) are used in an `IJavaBuilderContext` to collect all imports used.
...
The collector also knows about names which don't need an import (like the `String` class because the `java.lang` package is always automatically imported).
...
The main entry point to retrieve an API definition is the `org.eclipse.scout.sdk.core.java.apidef.Api` class.
...
The `core.typescript` module provide classes for two main topics:

. Parse TypeScript and JavaScript code into a model representation.
. Build new TypeScript and JavaScript source code.

These two topics are explained in the next chapters.

==== Parsing

The package `org.eclipse.scout.sdk.core.typescript.model` contains classes that represents JavaScript or TypeScript code as a parsed model.
It contains no JavaScript or TypeScript parser itself.Parses are contributed using a service provider interface (SPI).
Currently only one parser based on the WebStorm engine is available.
It is included in the Scout IntelliJ plugin (see <<s2i>>).
As a consequence the parser is only available in a running IntelliJ instance and is therefore also not available for the Eclipse plugins.

There are model classes available for the most important JavaScript or TypeScript elements (like classes, functions, variables).

===== Node Module

The root element is the `INodeModule` class which represents a single Node module.
It contains all other elements together with the `IPackageJson` that defines module.

===== Object Literals

JavaScript or TypeScript object literals are represented by the `IObjectLiteral` model class.
Here is a sample object literal assigned to a constant:

[source,javascript,indent=0]
.Example constant with an object literal as value.
----
export const VariableWithObjectLiteral = {
  text: 'left',
  missing: null,
  num: 3.21,
  nested: {
...
The following listing explains how to access the value of 'b' in this object literal:

[source,java,indent=0]
.Example how to access values of an object literal in an INodeModule.
----
include::{codedir}/ObjectLiteralSample.java[tags=objectLiteralSample]
----

<1> Gets the exported element from the node module
<2> As exported elements may be of various types (e.g. classes, functions, variables) it is converted to a variable here.
<3> Gets the value of the variable (right hand side)
<4> Converts the value to an object literal.
<5> The resulting Optional will be empty if the value is no object literal.
<6> Gets the value of the property named 'nested' as object literal throwing if it cannot be found or is no object literal.
<7> On this nested object literal gets the value of the property named 'b' throwing if no such property exists.
<8> Reads the value as boolean throwing if it is no valid boolean.
<9> Will print the value 'true'.
...
* DoContextResolver: Data object context resolvers may be contributed by implementing `IDoContextResolver` and activating using `DoContextResolvers#set`.
...
The context may respect texts available through Java dependencies (pom.xml) as well as Node dependencies (package.json).
* *Language*: Represents a single text language.
E.g. English (`en`) or British English (`en_GB`).
...
* `JavaEnvironmentExtension`: To specify `IJavaEnvironment` arguments in tests.
* `DefaultCommentGeneratorExtension`: To activate a default comment generator for Java elements.
* `UsernameExtension`: To set the context username to a fixed value (make output of user dependent code predictable).
* `ClassIdAutoCreationExtension`: If specified, enables automatic `@ClassId` creation.
* `TestingEnvironmentExtension`: To specify `TestingEnvironment` arguments in tests.
* `UniqueIdExtension`: To specify a fixed unique id value (make output of id dependent code predictable).
* `EnabledFor`: To only execute tests if a certain API is available (see <<API Definition>>).
* `ExtendWithDoContext`: To specify a data object context (namespace, type-version).
...
<1> Only executes the test if min the `IJavaApi` version 11 can be found in the `IJavaEnvironment` created by the `RunningJavaEnvironmentFactory`.
<2> Executes the given generator, registers the created source in the `IJavaEnvironment` (see <<From Generator to Model>>) and checks the generated source for compile errors.
